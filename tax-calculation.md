...
2. **价内税**：商品标价已经包含税额，税额 = 含税总价 − 不含税价格。由于含税总价是"整数金额"（顾客实际支付的金额），反推的不含税价格本身就由业务逻辑决定了精度。

**代码实际执行路径**：
- 每个商品独立调用 `get_included_tax()` 计算该商品的高精度税额（无舍入）
- 多个商品的税额通过 `update_taxes()` 以 4 位小数精度累加
- 最后通过 `round_taxes()` 对汇总后的税额做统一舍入

数学上，这与"先汇总总价再反推税额"是等价的（加法分配律）：
> Σ(含税价 − 含税价/(1+r)) = 总价 − 总价/(1+r)

但代码执行路径确实是**按商品逐个计算税额**，不是先汇总总价。不做单项舍入的设计原因是为了避免"反推税额相加 ≠ 总税额"的数学矛盾。
...
代码实际执行路径（正确）：
  商品 1 税额（高精度）= 11.504424...
  商品 2 税额（高精度）= 11.504424...
  累加（4 位）= 23.0088...
  最终舍入（2 位 HALF_UP）= 23.01 ✅
...
**正确的做法**：按商品计算高精度税额 → 累加 → 统一舍入。这就是价内税不在单项层面做舍入的设计原因。
...
### 7.9 单项税额的两个入口函数与舍入模式生效路径

#### 7.9.1 完整调用路径图

```
                           Tax_lib::get_taxes()  /  apply_destination_tax()
                                      │
                          tax_type == INCLUDED ?    (基础税制: config['tax_included'])
                                      │
                     ┌────────────────┴────────────────┐
                     │                                 │
                价内税 (INCLUDED)                  价外税 (EXCLUDED)
                     │                                 │
         get_included_tax()                  get_tax_for_amount()
                     │                                 │
              bcsub() 返回                    bcmul() → round_number()
              高精度结果                       → 按 tax_decimals 舍入
              (无单项舍入)                     → 触发 HALF_ODD / HALF_FIVE
                     │                                 │
                     └────────────┬────────────────────┘
                                  │
                        update_taxes() 累加 (4位小数)
                                  │
                          round_taxes() 最终舍入
                     (使用 rounding_code，但走另一套实现)
```

#### 7.9.2 两个入口函数的覆盖范围

| 函数 | 覆盖税种 | 舍入行为 | HALF_ODD 生效？ | HALF_FIVE 生效？ |
|------|---------|---------|---------------|----------------|
| [get_included_tax()](file:///d:/fz/0601-1/solo-dogfeeding/code/17-opensourcepos/app/Libraries/Tax_lib.php#L183-L189) | **仅价内税** | 无（`$tax_decimal`、`$rounding_code` 参数未使用） | ❌ 永不生效 | ❌ 永不生效 |
| [get_tax_for_amount()](file:///d:/fz/0601-1/solo-dogfeeding/code/17-opensourcepos/app/Libraries/Tax_lib.php#L76-L81) | **仅价外税** | 调用 `round_number()` 做单项舍入 | ✅ 生效 | ✅ 生效 |
| [round_taxes()](file:///d:/fz/0601-1/solo-dogfeeding/code/17-opensourcepos/app/Libraries/Tax_lib.php#L252-L294) | 价内 + 价外 | 内联 if-elseif 做最终舍入 | ⚠️ 错误实现 | ⚠️ 实现不一致 |

#### 7.9.3 HALF_ODD / HALF_FIVE 的生效条件链

要使 HALF_ODD 或 HALF_FIVE 在单项阶段生效，**必须同时满足以下所有条件**：

```
条件 1：use_destination_based_tax = true（目的地税制）
        └─ 基础税制下 rounding_code 硬编码为 HALF_UP，不触发

条件 2：tax_type = EXCLUDED（价外税）
        └─ 价内税走 get_included_tax()，该函数不做单项舍入

条件 3：tax_rates.tax_rounding_code = HALF_ODD(4) 或 HALF_FIVE(7)
        └─ 需要税率配置中显式设置该舍入模式

任一条件不满足 → HALF_ODD / HALF_FIVE 在单项阶段不生效
```

#### 7.9.4 各场景下 HALF_ODD / HALF_FIVE 的生效情况

| 场景 | 税制 | 税种类型 | rounding_code 配置 | 单项阶段 | 最终汇总阶段 |
|------|------|---------|-------------------|---------|------------|
| A | 基础 | 全部视为价外 | 强制 HALF_UP | ❌ 不触发 | ❌ 不触发 |
| B | 基础 | 全部视为价内 | 强制 HALF_UP（未使用） | ❌ 不触发 | ❌ 不触发 |
| C | 目的地 | 价外税 | HALF_UP | HALF_UP | HALF_UP |
| D | 目的地 | 价外税 | HALF_ODD | ✅ HALF_ODD | ⚠️ 实际执行 HALF_UP |
| E | 目的地 | 价外税 | HALF_FIVE | ✅ HALF_FIVE | ⚠️ 实现不一致 |
| F | 目的地 | 价内税 | HALF_ODD | ❌ 不触发（参数未使用） | ⚠️ 实际执行 HALF_UP |
| G | 目的地 | 价内税 | HALF_FIVE | ❌ 不触发（参数未使用） | ⚠️ 实现不一致 |

**关键结论**：
- HALF_ODD / HALF_FIVE 在单项阶段**只能在场景 D 和 E 下生效**（目的地税制 + 价外税）
- 价内税（场景 F、G）即使在税率配置中设置了 HALF_ODD 或 HALF_FIVE，单项阶段也不会执行，因为 `get_included_tax()` 内部未使用舍入参数
- 最终汇总阶段（`round_taxes()`）对所有税种统一处理，但 HALF_ODD 被错误实现为 HALF_UP，HALF_FIVE 的实现与单项阶段不一致
